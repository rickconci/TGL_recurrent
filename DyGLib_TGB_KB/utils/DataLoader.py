...
def get_link_prediction_tgb_data(dataset_name: str):
    ...
    years = 1
    time_difference = pd.Timedelta(days=91 + (365*years), hours=00, minutes=00)
    ...
    start_date = df_filtered['timestamp'].min().date()
    end_date = df_filtered['timestamp'].max().date()
    ...
    for day in range(total_days):
        start = start_date + pd.Timedelta(days=day)
        end = start  # We're focusing on a single day

        day_pairs = df_filtered[(df_filtered['Date'] >= start) & (df_filtered['Date'] <= end)]
        pairs_count = day_pairs.groupby(['source', 'destination']).size().reset_index(name=f'Day_{day+1}_Counts')

        pairs_count = pairs_count[pairs_count[f'Day_{day+1}_Counts'] >= 2]

        if not pairs_count.empty:
            daily_pairs.append(pairs_count)

    common_pairs = daily_pairs[0][['source', 'destination']]
    for day_pairs in daily_pairs[1:]:
        common_pairs = pd.merge(common_pairs, day_pairs[['source', 'destination']], on=['source', 'destination'], how='inner')

    data = df_filtered.merge(common_pairs, on=['source', 'destination'])

    name = f'Flight_day_2_short'
    ...
    eval_neg_edge_generator = NegativeEdgeGenerator(name, dst_node_ids[0], dst_node_ids[len(dst_node_ids)-1])
    path = '/home/kmb85/rds/hpc-work/L65_Practical/DyGLib_TGB/neg_samplers'
    eval_neg_edge_generator.generate_negative_samples(full_data, 'val', path)
    eval_neg_edge_sampler = NegativeEdgeSampler(name)
    eval_neg_edge_sampler.load_eval_set(path+f'/{name}_val_ns.pkl')
