...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            max_len = max(len(item) for item in batch_neg_dst_node_ids_list)
            padded_list = [item + [0]*(max_len - len(item)) for item in batch_neg_dst_node_ids_list]
            # Convert to a NumPy array and add 1
            batch_neg_dst_node_ids = np.array(padded_list) + 1
