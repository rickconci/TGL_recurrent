...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   ...
                                   eval_metric_name: str, evaluator: LinkPredictionEvaluator, num_neighbors: int = 8, time_gap: int = 2000):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            #save the destination nodes quered in evaluation 
            #for sublist in batch_neg_dst_node_ids.tolist():
            #    adjusted_sublist = [item - 1 for item in sublist]
            #    dest_nodes.extend(adjusted_sublist)
            #save the source nodes queried in evaluation 
            #source_nodes.extend(repeated_batch_src_node_ids.tolist())
            #save the timestamps queried in evaluation 
            #time_pred.extend(repeated_batch_node_interact_times.tolist())
            ...
            for sample_idx in range(len(batch_src_node_ids)):
                ...
                #print('ypred_pos', input_dict['y_pred_pos'].shape)
                #print('ypred_neg', input_dict['y_pred_neg'].shape)
                y_pred = np.concatenate((input_dict['y_pred_pos'], input_dict['y_pred_neg']))
                pred_vals.extend(y_pred.tolist())

                source_nodes.append(batch_src_node_ids[sample_idx] - 1)  
                dest_nodes.append(batch_dst_node_ids[sample_idx] - 1) 
                time_pred.append(batch_node_interact_times[sample_idx])


                for neg_dest in batch_neg_dst_node_ids[sample_idx]:
                    source_nodes.append(batch_src_node_ids[sample_idx]-1)
                    dest_nodes.append(neg_dest-1)
                    time_pred.append(batch_node_interact_times[sample_idx])

            print('dest_nodes', len(dest_nodes))
            print('source_nodes', len(source_nodes))
            print('time_pred', len(time_pred))
            print('pred_vals', len(pred_vals))
    ...
    df_res = pd.DataFrame({
            'source': source_nodes,
            'destination': dest_nodes,
            'timestamp': time_pred,
            'predictions': pred_vals
        })
    
    path_name_to_save = os.path.join('saved_results', model_name, 'tgbl-synthetic', eval_stage)
    if not os.path.exists(path_name_to_save):
        os.makedirs(path_name_to_save)

    df_res.to_csv(path_name_to_save + '_synthetic_validation_data.csv', index=False)
