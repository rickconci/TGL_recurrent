...
class NegativeEdgeGenerator(object):
    def __init__(
        ...
        assert strategy in [
            ...
            "hist_rnd",
            "all-possible"
        ], "The supported strategies are `rnd` or `hist_rnd` and 'all-possible'!"
        ...
        elif self.strategy =='all-possible':
            assert (
                historical_data != None
            ), "Train data should be passed when `hist_rnd` strategy is selected."
            self.historical_data = historical_data
    ...
    def generate_negative_samples(self, 
        ...
        if self.strategy == "rnd":
            ...
            self.generate_all_pairs_negatives(data, self.historical_data, split_mode, filename)
        else:
            raise ValueError("Unsupported negative sample generation strategy!")
        
    def generate_all_pairs_negatives(self, 
                                     data: TemporalData, #validation, test
                                     historical_data: TemporalData, #train
                                     split_mode: str, 
                                     filename: str):
        
        print(f"INFO: Generating ALL-PAIRS negative samples for '{split_mode}'")
        if os.path.exists(filename):
            print(f"INFO: ALL-PAIRS negatives for '{split_mode}' already generated!")
        else:
            train_sources = historical_data.src.cpu().numpy()
            train_destinations = historical_data.dst.cpu().numpy()
            #print('train_sources', train_sources.shape)
            #print('train_destinations', train_destinations.shape)
            unique_sources = np.unique(train_sources)
            unique_destinations = np.unique(train_destinations)
            ...
            #evaluation_set = {}
            #for ts in tqdm(unique_timestamps, desc="Timestamps"):
            #    for src in all_sources:
            #        for dst in all_destinations:
            #            # Exclude positive edges already in `data`
            #            if not ((data.src == src) & (data.dst == dst) & (data.t == ts)).any():
            #                key = (src, dst, ts)
            #                evaluation_set[key] = 1  # Or any other placeholder value
